...
(R 4.4.1) `r-minimal` image is 23.74MB compressed, and 45.88MB
uncompressed.

```{r, include = FALSE}
source("tools/alpine-version.R")
```

All current images use Alpine Linux `r alpine_version()`. Older images use
...
* No X11 support.
* No OpenMP support. (But you can configure it for a package, see [examples/data.table](examples/data.table).)
...
| R 4.5.x   | `r rv("4.5")`     | `r rv_tags("4.5")`     |             |
| R 4.4.x   | `r rv("4.4")`     | `r rv_tags("4.4")`     |             |
...
We tag our daily R-devel builds with the build date.
These images can be useful for tracking down bugs and
regressions in R. E.g.:
```sh
docker run -ti ghcr.io/r-hub/r-minimal/r-minimal:2022-11-25
```
The tags start on 2021-07-25. `linux/arm64` images are available
from 2022-01-13.
...
Please refer to [examples/renv-shiny](examples/renv-shiny) for an example that install shiny and rmarkdown in a container.
...
* The arrow package needs a `Makevars` file to add a link flag. See the
  example `Dockerfile` in the [examples/arrow](examples/arrow) directory.
