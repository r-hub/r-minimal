...
If you copy the entire folder with renv, including the `activate.R`, renv will bootstrap itself with the same version as the lock and restore the packages with the proper versions.
All the necessary compilers and libraries needed at runtime need to be installed with the `-a` and `-t` options.
Please refer to examples/shiny-renv for an example that install shiny and rmarkdown in a container. 
